...
public class LightsTests 
{
	...
	private static final double KD = 0.5; //diffuse coefficient
	private static final Double3 KD3 = new Double3(0.2, 0.6, 0.4);

	private static final double KS = 0.5; //specular coefficient
	...
	/**
	Test case for rendering triangles with lights.
	Adds triangles with specified materials and adds different types of lights to the scene.
	Sets up the camera, image writer, and ray tracer to render the scene and save the image.
	*/
	@Test
	public void trianglesLights() 
	{
		// Add triangles with materials to the scene
		scene2.geometries.add(triangle1.setMaterial(new Material().setkD(0.5).setkS(0.5).setnShininess(350)),
				triangle2.setMaterial(new Material().setkD(0.5).setkS(0.5).setnShininess(350)));
		// Add a spot light to the scene
		scene2.lights.add(new SpotLight(new Color(500, 250, 250), new Point(-10, 10, -100), new Vector(1, -1, -1))
				.setkL(0.0001).setkQ(0.000005));
		// Add a point light to the scene
		scene2.lights.add(new PointLight(new Color(250, 500, 250), new Point(10, -20, -80))
				.setkL(0.0005).setkQ(0.0005));
		// Add a directional light to the scene
		scene2.lights.add(new DirectionalLight(new Color(150, 150, 500), new Vector(-1, 2, -1))); //can change the background color
	
		// Set up the image writer
		ImageWriter imageWriter = new ImageWriter("lightsInTriangles", 500, 500);
	...
	/**
		Test case for rendering a sphere with lights.
		Adds a sphere to the scene and adds different types of lights to illuminate the sphere.
		Sets up the camera, image writer, and ray tracer to render the scene and save the image.
	 */
	@Test
	public void sphereLights() 
	{
		...
		scene1.geometries.add(sphere);
		// Add a directional light to the scene (purplish)
		scene1.lights.add(new DirectionalLight(new Color(200,0,200), new Vector(0, 1, -1))); //purplish 
		// Add a spot light to the scene (bluish-green)
		scene1.lights.add(new SpotLight(new Color(0, 200, 200), new Point(50, 50, 50), new Vector(-1, -1, -2)) //a bluish-green
				.setkL(0.00001).setkQ(0.000000001));
		// Add a point light to the scene (yellowish)
		scene1.lights.add(new PointLight(new Color(500, 300, 0), new Point(-50, 50, 50))//yellowish 
				.setkL(0.0001).setkQ(0.000001));
		// Set up the image writer
		ImageWriter imageWriter = new ImageWriter("lightsInSphere", 500, 500);
		// Set up the camera with the image writer and ray tracer
				camera1.setImageWriter(imageWriter)
				.setRayTracerBase(new RayTracerBasic(scene1))
		        .renderImage()
